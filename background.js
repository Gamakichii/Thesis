...
// FIXED: Aggregates per-link predictions using batch endpoint with better error handling
async function getMLPrediction(postText, postLinks, postId) {
  try {
    ...
    console.log(`🔍 Analyzing ${postLinks.length} links for post ${postId}:`, postLinks);
    ...
    if (!response.ok) {
      ...
      console.error(`❌ Batch prediction failed: ${response.status} - ${errorText}`);
      throw new Error(`batch status ${response.status}`);
    }
    ...
    const result = { isPhishing: flagged.length > 0, flaggedLinks: flagged };
    console.log(`🎯 ML Result for post ${postId}:`, result);
    
    return result;
  } catch (e) {
    console.warn('⚠️ Batch prediction failed, falling back per-link', e);
    
    // Fallback to individual predictions
    const results = await Promise.all(postLinks.map(async (link) => {
      const res = await getBackendPredictionForLink(link, postId);
      console.log(`🔗 Individual prediction for ${link}:`, res);
      return { link, res };
    }));
    
    const flaggedLinks = results.filter(({ res }) => !!res && res.is_phishing).map(({ link }) => link);
    const result = { isPhishing: flaggedLinks.length > 0, flaggedLinks };
    console.log(`🎯 Fallback ML Result for post ${postId}:`, result);
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "scanPage") {
    ...
    request.posts.forEach(async (post) => {
      try {
        console.log(`🔍 Processing post ${post.id} with ${post.links.length} links`);
        
        const prediction = await getMLPrediction(post.text, post.links, post.id);
        
        if (prediction.isPhishing) {
          console.warn(`🚨 PHISHING DETECTED in post ${post.id}! Flagged links: ${prediction.flaggedLinks.join(', ')}`);
          ...
          chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            if (tabs[0]) {
              console.log(`📤 Sending blur command for post ${post.id}`);
              chrome.tabs.sendMessage(tabs[0].id, { action: "blurPost", postId: post.id }, (response) => {
                if (chrome.runtime.lastError) {
                  console.error(`❌ Failed to send blur command: ${chrome.runtime.lastError.message}`);
                } else {
                  console.log(`✅ Blur command sent successfully:`, response);
                }
              });
            }
          });
        } else {
          console.log(`✅ Post ${post.id} is safe - no phishing detected`);
...
function triggerScanOnActiveFacebookTab() {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    console.log('🔍 Auto-triggering scan on Facebook tab:', tab.url);
